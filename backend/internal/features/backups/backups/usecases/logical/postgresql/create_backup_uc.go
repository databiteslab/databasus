...
func (uc *CreatePostgresqlBackupUsecase) buildPgDumpArgs(pg *pgtypes.PostgresqlLogicalDatabase) []string {
	...
	for _, table := range pg.ExcludeTables {
		args = append(args, "--exclude-table="+normalizePgDumpTablePattern(table))
	}

	for _, table := range config.GetEnv().BackupExcludeTables {
		args = append(args, "--exclude-table="+normalizePgDumpTablePattern(strings.TrimSpace(table)))
	}

	for _, table := range config.GetEnv().BackupIncludeTables {
		args = append(args, "--table="+normalizePgDumpTablePattern(strings.TrimSpace(table)))
	}
